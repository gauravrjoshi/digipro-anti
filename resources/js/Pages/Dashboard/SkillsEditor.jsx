...
const SkillsEditor = () => {
    ...
    return (
        <div>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '2rem' }}>Professional Skills</h2>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', padding: '2rem', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '3rem' }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: 'bold', marginBottom: '1.5rem' }}>Add New Skill</h3>
                <form onSubmit={handleCreate} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                    <div className="form-group">
                        <label style={labelStyle}>Skill Name</label>
                        <input value={data.name} onChange={e => setData('name', e.target.value)} style={inputStyle} placeholder="e.g. React" required />
                    </div>
                    <div className="form-group">
                        <label style={labelStyle}>Proficiency (%)</label>
                        <input type="number" value={data.proficiency} onChange={e => setData('proficiency', e.target.value)} style={inputStyle} min="0" max="100" required />
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                        <button type="submit" disabled={processing} style={{ width: '100%', padding: '0.8rem', background: '#667eea', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>
                            Add Skill
                        </button>
                    </div>
                </form>
                    ...
                    <div key={skill.id} style={{ background: 'rgba(255,255,255,0.03)', padding: '1.25rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', paddingRight: '1rem' }}>
                                ...
                                <span style={{ color: '#94a3b8', fontSize: '0.875rem' }}>{skill.proficiency}%</span>
                            </div>
                            <div style={{ height: '6px', background: 'rgba(255,255,255,0.1)', borderRadius: '3px', marginRight: '1rem' }}>
                                <div style={{ width: `${skill.proficiency}%`, height: '100%', background: '#667eea', borderRadius: '3px' }}></div>
                            </div>
                        </div>
                        <button onClick={() => handleDelete(skill.id)} style={{ padding: '0.5rem', background: 'transparent', color: '#ef4444', border: 'none', cursor: 'pointer', fontSize: '0.8rem' }}>Delete</button>
                    </div>
                ))}
            </div>
        </div>
...
const inputStyle = { width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: 'white', outline: 'none' };
